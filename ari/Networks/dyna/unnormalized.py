...
for n in range(n_batch):
    ...
    for k in range(batch_size//n_images):
        ...
        if i%2==0:
            ...
            average_weight_norm = tf.reduce_mean(tf.sqrt(tf.reduce_sum(W_up**2,[0,1,2])))
            average_activity = tf.reduce_mean(a.activity())
            run_log.write("Mean norm of weight vectors: "+str(average_weight_norm.numpy()))
            run_log.write("Mean activity: "+str(average_activity.numpy()))
        # Increment counter
        if i==200:
            assert(0)
        i += 1
        ...
        for step in range(steps_per_image):
            ...
            dW1 = learning_rate*hebb(E.activity(),a.activity(),filter_size+(n_features,),stride)  #Original
            dW2 = -(5e-6)*tf.reduce_sum(a.activity(),[0,1,2],keepdims=True)

            W_down.assign_add(dW1+dW2*W_down)
            W_up.assign_add(dW1+dW2*W_up)
